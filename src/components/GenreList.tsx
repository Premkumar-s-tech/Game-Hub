...
const GenreList = ({ onSelectGenre, selectedGenre}: Props) => {
  ...
  return (
    <>
    <Heading fontSize='2l' marginBottom={3}>Genres</Heading>
    <List>
        {data.map(genre => <ListItem key={genre.id} paddingY='5px'><HStack>
          <Image boxSize='32px' borderRadius={8} objectFit='cover' src={getCroppedImageUrl(genre.platforms[0].image_background)}/>
